"""Builds assets/stats-{dark,light}.svg: the streak + languages card.
...
def glitch_style(t, name, period):
    """RGB-split layers that sit slightly apart and jump around for a moment every `period` seconds."""
    return f"""
    .{name}-cyan {{ fill: {t['cyan']}; mix-blend-mode: {t['blend']}; transform: translate(-2px, 0); animation: {name}-cyan {period}s steps(1) infinite; }}
    .{name}-red {{ fill: {t['red']}; mix-blend-mode: {t['blend']}; transform: translate(2px, 0); animation: {name}-red {period}s steps(1) infinite; }}
    .{name}-base {{ fill: {t['fg']}; animation: {name}-base {period}s steps(1) infinite; }}
    @keyframes {name}-cyan {{
...
def card(t, current, longest, total, langs):
    ...
    return f"""<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 176" width="800" height="176" role="img" aria-label="{current} day contribution streak">
  ...
  <use href="#num" class="num-cyan"/>
  <use href="#num" class="num-red"/>
  <use href="#num" class="num-base"/>
